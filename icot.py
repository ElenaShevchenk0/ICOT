...
class InterpretableClusteringOptimalTree:
    ...
    def _constructMIO(self, x):
        ...
        # constraints
        m.addConstrs(s[i] * M[i] == (q[i] - r[i])  for i in range(self.n))
        m.addConstrs(q[i] <= M[i] for i in range(self.n))
        m.addConstrs(r[i] <= M[i] for i in range(self.n))
        m.addConstrs(gp.quicksum(c[i,t] * z[i,t] for t in self.l_ind) == r[i] for i in range(self.n))
        m.addConstrs(K[t] == gp.quicksum(z[i,t] for i in range(self.n)) for t in self.l_ind)
        m.addConstrs(gp.quicksum(dist[i,j] * z[j,t] for j in range(self.n)) + np.sqrt(self.p)*(1 - l[t]) == c[i,t] * (K[t] + (1 - l[t])) for t in self.l_ind for i in range(self.n))
        m.addConstrs(d[t] == gp.quicksum(a[j,t] for j in range(self.p)) for t in self.b_ind)
        m.addConstrs(b[t] <= d[t] for t in self.b_ind)
        m.addConstrs(d[t] <= d[t//2] for t in self.b_ind if t != 1)
    ...
    @staticmethod
    def _comp_dist(x):
        """compute eucledian distance between all observations"""
        rows = x.shape[0]
        clmns = x.shape[1]
